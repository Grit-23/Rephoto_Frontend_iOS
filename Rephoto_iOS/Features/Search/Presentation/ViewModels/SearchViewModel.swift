...
final class SearchViewModel {
    ...
    /// 검색 결과. "결과 없음"과 "검색 실패"를 case로 갈라 둔다
    private(set) var searchResults: Loadable<[SearchResult]> = .idle
    ...
    func search(query: String) async {
        ...
        do {
            ...
            guard generation == searchGeneration, !Task.isCancelled, !error.isCancellation else { return }
            searchResults = .failed(AppError.from(error))
        }

        // 이전에 색인 로드가 실패했다면 결과 → 상세 매핑을 위해 재시도
        // (결과 반영 후에 수행 — 재시도 동안 스피너를 붙잡지 않는다)
