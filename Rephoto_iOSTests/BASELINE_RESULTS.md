...
> 다만 **처리 시간은 실기기에서도 2016 정렬이 빠르다** (0.027s → 0.019s, **−30%**).
...
**반면 디스플레이 경로로 디코드를 강제하면(B) 1.48B/px** — YUV 4:2:0(1.5B/px)에 근접한다
→ **가설 (b)**. 시뮬레이터 Debug에서 D가 +19.1MB로 나왔던 것도 18.3MB(YUV 4:2:0)에 가깝다.
플랫폼에 따라 D가 YUV 버퍼를 상주시키기도, 안 하기도 한다.

**그리고 C가 반증을 닫는다.** 픽셀 포맷을 RGBA로 못박으면 46.6MB — 이론값과 0.2% 오차로
일치한다. 디코더가 46.5MB를 못 만드는 게 아니라, **대조군이 그걸 요구한 적이 없다.**

> **결론: `test_fullDecodeControl_peakDelta`는 이름이 틀렸다.** 풀디코드 대조군이 아니라
> "다운샘플 없는 디코드+재인코딩 경로"이고, 그 메모리는 대부분 출력 JPEG이다.
> 이 벤치를 "풀디코드 대비 −N%" 같은 메모리 개선 근거로 쓰면 안 된다.
> (#34의 실증된 효과는 여전히 **페이로드 −73% + 처리 시간 −30%**이다.)
