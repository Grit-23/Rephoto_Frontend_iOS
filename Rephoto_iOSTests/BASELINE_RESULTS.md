...
이 최적화(#34+#49)의 실증된 효과는 **페이로드 −73% + 처리 시간 −22%(A16)·−24%(A13)**이며
...
> `PhotoMetadataExtractor.swift:89`)이므로 **#49의 개선폭으로 인용할 수 있는 값은 A→D = −22%**다.
...
같은 기기 안의 비교라 칩 성능이 약분되므로, **프로덕션 개선폭 −22~24%는 세대 무관한 실효 개선**이다.
...
> (#34+#49의 실증된 효과는 **페이로드 −73% + 처리 시간 −22%(A16)·−24%(A13)**이다 —
