...
//  - 입력 검증(빈 필드) 시 도메인 에러, 세션 호출 없음
//  - 로그인 성공/실패 시 isLoading/error 상태 전이
//
...
final class LoginViewModelTests: XCTestCase {
    ...
    func test_login_emptyFields_setsValidationError_withoutCallingSession() async {
        ...
        XCTAssertEqual(sut.error, .domain(.emptyCredentials))
    ...
    func test_login_failure_setsError_andEndsLoading() async {
    ...
    func test_isShowingError_setFalse_clearsError() {
        sut.error = .unknown(message: "에러")
